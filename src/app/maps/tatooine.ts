...
export class Tatooine {
  ...
  public tatooineObjects = {
    ...
    checkpointObjects: [
      new CheckpointObject({ctx: this.ctx, x: 500, y: 250, width: 10, height: 50, color: 'blue'}),
    ],
    enemyObjects: [
      new EnemyObject({ctx: this.ctx, x: 900, y: 400, width: 50, height: 50, color: 'red'})
    ],
    finishObject: new FinishObject({ctx: this.ctx, x: 780, y: 500, width: 10, height: 50, color: 'pink'})
  };
}
